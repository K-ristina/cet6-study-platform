...
本项目已经原生适配 **Vercel**，支持零配置一键上线：

1. 将代码推送到 GitHub 仓库；
2. 登录 [Vercel 控制台](https://vercel.com/)，选择 **Import Git Repository** 导入本项目；
3. Framework Preset 选择 **Vite**，直接点击 **Deploy**；
4. 项目自带的 `api/dict.js` 和 `vercel.json` 会由 Vercel 自动识别并部署为边缘 Serverless 云函数，无需手动配置跨域或后端服务器。
